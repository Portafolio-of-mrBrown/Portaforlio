...
export class SupplierService {
  ...
  supplierWithConcatMap$ = of(1, 5, 8)
    .pipe(
      tap(id => console.log('concatMap source Obserbvable', id)),
      concatMap(id => this.http.get<Supplier>(`${this.suppliersUrl}/${id}`))
    );

  supplierWithMergeMap$ = of(1, 5, 8)
    .pipe(
      tap(id => console.log('mergeMap source Obserbvable', id)),
      mergeMap(id => this.http.get<Supplier>(`${this.suppliersUrl}/${id}`))
    );

  constructor(private http: HttpClient) {

    // this.suppplierWithMap$.subscribe(item => console.log('map result', item));
